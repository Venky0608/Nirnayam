    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "32px 20px" }}>
      <div style={{ width: "100%", maxWidth: 560 }}>
        ...
        <div key={step} style={{ animation: "fadeIn 0.3s ease forwards" }}>
          ...
          {q.type === "scale" && (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", fontFamily: mono, fontSize: 13, color: "#777", marginBottom: 18 }}>
              ...
              <div style={{ textAlign: "center", marginTop: 16, fontFamily: syne, fontSize: 52, fontWeight: 800, color: "#fff" }}>{answers[q.id]}</div>
            </div>
          )}
            ...
            <div>
              {(answers.grade === "Grade 11" || answers.grade === "Grade 12") && (
                <div style={{ marginBottom: 24 }}>
                  <div style={{ fontFamily: mono, fontSize: 12, color: "#666", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 12 }}>Which stream are you in?</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {STREAM_OPTIONS.map(s => <button key={s} onClick={() => { setStream(s); setAnswers(a => ({ ...a, optionalSubjects: [] })); }} style={{ ...btnStyle(stream === s), padding: "13px 16px" }}>{s}</button>)}
                  </div>
                </div>
              )}
              {compulsory.length > 0 && (
            ...
            <div>
              {answers.subjectPriority.length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  <div style={{ fontFamily: mono, fontSize: 12, color: "#444", marginTop: 8 }}>Drag to reorder on desktop · Use arrows on mobile</div>
                </div>
              )}
            </div>
          )}
          ...
          {q.type === "learning" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              ...
              <div>
                <div style={{ fontFamily: mono, fontSize: 14, color: "#888", marginBottom: 12 }}>How long does revising one subject typically take you?</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {q.type === "text" && (
            <textarea value={answers[q.id] || ""} onChange={e => setAnswers(a => ({ ...a, [q.id]: e.target.value }))} placeholder={q.placeholder || "Type here..."}
    ...
    <div style={{ animation: "fadeIn 0.4s ease forwards" }}>
      ...
      <div style={{ background: "#0f0f0f", border: "1px solid #1a1a1a", borderRadius: 8, padding: "18px", marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", fontFamily: mono, fontSize: 13 }}>
          <span style={{ color: "#ccc" }}>Primary — {splitA}%</span>
          <span style={{ color: "#555" }}>Secondary — {splitB}%</span>
        </div>
      </div>
