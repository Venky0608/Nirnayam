...
RULES: One clear recommendation only. Direct like a smart older sibling. Use subject priority order to break ties. Flag urgency clearly. Be decisive and concise. Use strong, direct language without being harsh. Avoid vague phrases. Be specific and actionable.
...
// ─── Firebase helpers ─────────────────────────────────────────────────────────
const saveProfileToFirestore = async (uid, profile) => {
...
// ─── Landing Page ─────────────────────────────────────────────────────────────
function LandingPage({ onGoogleSignIn, onGuestStart, authLoading }) {
      ...
      <div style={{
        ...
        {!expanded ? (
          ...
          <div style={{ fontFamily: mono, fontSize: 14, color: "#888", marginBottom: 36, lineHeight: 1.9, textAlign: "left", background: "#0d0d0d", border: "1px solid #1e1e1e", borderRadius: 8, padding: "24px" }}>
            ...
            <p style={{ marginBottom: 16 }}>Nirnayam will give you a clear recommendation, show you how confident it is, provide a step-by-step action plan, and flag anything to watch out for.</p>
        ...
        {/* Google Sign In */}
        <button onClick={onGoogleSignIn} disabled={authLoading} style={{
          background: "#fff", color: "#000", border: "none", borderRadius: 5,
          padding: "17px 44px", fontFamily: mono, fontSize: 15, fontWeight: 500,
          cursor: authLoading ? "not-allowed" : "pointer", letterSpacing: "0.05em",
          transition: "all 0.2s", display: "block", margin: "0 auto 12px",
          WebkitTapHighlightColor: "transparent", opacity: authLoading ? 0.6 : 1,
        }}
        ...
        {/* Guest */}
        <button onClick={onGuestStart} style={{
          background: "transparent", color: "#777", border: "1px solid #333",
          borderRadius: 5, padding: "13px 36px", fontFamily: mono, fontSize: 13,
          cursor: "pointer", letterSpacing: "0.05em", display: "block", margin: "0 auto",
          WebkitTapHighlightColor: "transparent",
        }}>
          Continue without account
        </button>
 
        <div style={{ marginTop: 14, fontFamily: mono, fontSize: 11, color: "#2a2a2a" }}>
          Sign in to save your profile across devices
        </div>
 
        <div style={{ marginTop: 8, fontFamily: mono, fontSize: 12, color: "#333" }}>
        ...
        <div key={step} style={{ animation: "fadeIn 0.3s ease forwards" }}>
          ...
          {q.type === "choice" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {q.options.map(opt => (
                <button key={opt} onClick={() => setAnswers(a => ({ ...a, [q.id]: opt }))} style={btnStyle(answers[q.id] === opt)}>{opt}</button>
              ))}
            </div>
          )}
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              {q.options.map(opt => {
                ...
                return (
                  ...
                  }} style={{ ...btnStyle(sel), padding: "12px 16px" }}>{opt}</button>
                );
              })}
            </div>
          )}
            ...
            <div>
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {STREAM_OPTIONS.map(s => (
                      <button key={s} onClick={() => { setStream(s); setAnswers(a => ({ ...a, optionalSubjects: [] })); }} style={{ ...btnStyle(stream === s), padding: "12px 16px" }}>{s}</button>
                    ))}
                  </div>
                </div>
              )}
              {compulsory.length > 0 && (
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {compulsory.map(sub => (
                      <div key={sub} style={{ background: "#111", border: "1px solid #222", borderRadius: 5, padding: "10px 14px", fontFamily: mono, fontSize: 13, color: "#666" }}>{sub} ✓</div>
                    ))}
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {optionalList.map(sub => (
                      <button key={sub} onClick={() => handleOptionalSubject(sub)} style={{ ...btnStyle(answers.optionalSubjects.includes(sub)), padding: "12px 14px" }}>{sub}</button>
                    ))}
                  </div>
                </div>
              )}
...
// ─── Main App ─────────────────────────────────────────────────────────────────
function MainApp({ profile, user, onEditProfile, onSignOut }) {
    ...
    <div style={{ minHeight: "100vh", padding: "28px 20px", maxWidth: 660, margin: "0 auto" }}>
      ...
      {showHistory ? (
        ...
        <>
          ...
          {error && (
            <div style={{ background: "#1a0a0a", border: "1px solid #2a1010", borderRadius: 6, padding: "14px 18px", fontFamily: mono, fontSize: 13, color: "#f87171" }}>{error}</div>
          )}
...
// ─── Settings Page ────────────────────────────────────────────────────────────
function SettingsPage({ profile, user, onEditProfile, onSignOut, onBack }) {
  return (
    <div style={{ minHeight: "100vh", padding: "32px 20px", maxWidth: 540, margin: "0 auto" }}>
      ...
      <div style={{ background: "#0d0d0d", border: "1px solid #1e1e1e", borderRadius: 8, padding: "20px", marginBottom: 12 }}>
        ...
        {user ? (
          ...
          <div style={{ fontFamily: mono, fontSize: 13, color: "#666" }}>
            You're using Nirnayam as a guest. Sign in to save your profile across devices.
          </div>
        )}
      </div>
      ...
      <div style={{ fontFamily: mono, fontSize: 11, color: "#2a2a2a", textAlign: "center", marginTop: 8 }}>
        Editing will re-run the onboarding questionnaire
      </div>
    </div>
  );
}
...
function HistoryView({ onSelect }) {
  if (_history.length === 0) {
    return <div style={{ textAlign: "center", padding: "60px 0", fontFamily: mono, fontSize: 14, color: "#2a2a2a" }}>No decisions yet.</div>;
  }
  return (
    <div>
      <div style={{ fontFamily: mono, fontSize: 11, color: "#2a2a2a", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 16 }}>Past decisions</div>
      {_history.map((h, i) => (
...
export default function Nirnayam() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        try {
          const savedProfile = await loadProfileFromFirestore(firebaseUser.uid);
          if (savedProfile) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080808", color: "#e5e5e5" }}>
      ...
      {screen === "landing" && (
        <LandingPage onGoogleSignIn={handleGoogleSignIn} onGuestStart={handleGuestStart} authLoading={authLoading} />
      )}
